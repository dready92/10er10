...
function getAndParseByAlbum(couchQuery, ignoredAlbums = []) {
      ...
      rows.forEach(row => {
        ...
        if (song.album) {
          ...
          if (ignoredAlbums.indexOf(song.album) < 0) {
            const album = getAlbum(name, albums);
            if (album.isNew) {
              album.isNew = false;
              if (!totalReached) {
                byAlbum.push(album);
  ...
  function incrementAndExpiresAlbumTTL(albums, albumMaxTTL) {
    const response = { ...albums };
    response.keys().map(name => response[name])
      .forEach(album => {
        album.ttl = album.ttl + 1;
